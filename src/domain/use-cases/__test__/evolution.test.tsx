...
describe('EvolutionDataSource', () => {
    ...
    it('should get an evolution by name', async () => {
        ...
        await waitFor(() => {
            expect(result.current.data).toEqual({
                data: { id: 1, name: 'evolution1' }
            });
        });
    });

    it('should get a pokemon species', async () => {
        const mockResponse = { data: { id: 1, name: 'pokemon1', evolution_chain: { url: "" } } };
    ...
    it('should get a pokemon evolution chain by pokemon name', async () => {
        ...
        const wrapper = ({ children }) => (<QueryClientProvider client={queryClient}> {children} </QueryClientProvider>);
        const { result: species } = renderHook(() => usePokemonGetSpecies('pokemon1'), { wrapper });
        const { result } = renderHook(() => useEvolutionChainByPokemonName('pokemon1'), { wrapper });

        await waitFor(() => {
            expect(species.current?.data?.data).toEqual(mockSpeciesResponse.data);
